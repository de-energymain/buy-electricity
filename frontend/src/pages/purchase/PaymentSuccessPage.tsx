...
function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    if (location.state) {
      ...
      // Use real transaction signature if available, otherwise generate a fake one for testing
      if (stateData.signature) {
        setEntireHash(stateData.signature);
        // Format hash to look like the one in the design (truncated with ellipsis)
        const displayHash =
          stateData.signature.length > 20
            ? `${stateData.signature.substring(
                0,
                12
              )}...${stateData.signature.substring(
                stateData.signature.length - 6
              )}`
            : stateData.signature;
        setTransactionHash(displayHash);
      } else {
        // Fallback: Generate transaction hash for testing if no real signature
        console.warn(
          "No transaction signature provided, generating fake hash for testing"
        );
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card className="w-full bg-[#222]">
                  <CardBody className="p-4">
                    ...
                    <div className="text-white">
                      {orderDetails.tokenAmount.toLocaleString()}{" "}
                ...
                <Card className="w-full bg-[#222]">
                  <CardBody className="p-4">
                    ...
                    <div className="text-white font-mono">{nodeToken}</div>
                  </CardBody>
                </Card>
              </div>
            </div>
